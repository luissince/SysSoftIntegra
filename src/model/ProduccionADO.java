        ...
        try {
            ...
            if (statementValidate.executeQuery().next()) {
                ...
                if (statementValidate.executeQuery().next()) {
                    ...
                    statementProducion = DBUtil.getConnection().prepareStatement("UPDATE ProduccionTB SET Dias = Dias + ?, Horas = Horas + ?, Minutos = Minutos + ?,CostoAdicioanal = ?,Cantidad = ?,CantidadVariable = ?,Estado = ?,Descripcion=? WHERE IdProduccion = ?");
                    ...
                    statementProduccionDetalle = DBUtil.getConnection().prepareStatement("INSERT INTO ProduccionDetalleTB(IdProduccion,IdProducto,Cantidad,Costo,Medida)VALUES(?,?,?,?,?)");
                    ...
                    statementSuministroKardex = DBUtil.getConnection().prepareStatement("INSERT INTO KardexSuministroTB(IdSuministro,Fecha,Hora,Tipo,Movimiento,Detalle,Cantidad,Costo,Total,IdAlmacen,IdEmpleado)VALUES(?,?,?,?,?,?,?,?,?,?,?)");
                    statementInventario = DBUtil.getConnection().prepareStatement("UPDATE SuministroTB set Cantidad = (Cantidad - ?) where IdSuministro = ?");
                    statementInventarioKardex = DBUtil.getConnection().prepareStatement("INSERT INTO KardexSuministroTB(IdSuministro,Fecha,Hora,Tipo,Movimiento,Detalle,Cantidad,Costo, Total,IdAlmacen,IdEmpleado)VALUES(?,?,?,?,?,?,?,?,?,?,?)");
                    statementMerma = DBUtil.getConnection().prepareStatement("INSERT INTO MermaTB(IdMerma,IdProduccion,IdUsuario) VALUES(?,?,?)");
                    statementMermaDetalle = DBUtil.getConnection().prepareStatement("INSERT INTO MermaDetalleTB(IdMerma,IdProducto,IdTipoMerma,Cantidad,Costo,Medida) VALUES(?,?,?,?,?,?)");
                ...
                statementCorrelativo = DBUtil.getConnection().prepareStatement("SELECT MAX(Correlativo) AS Correlativo FROM ProduccionTB");
                int correlativo = 1;
    ...
    public static Object ListarProduccion(int tipo, String fechaInicio, String fechaFinal, String busqueda, int estado, int posicionPagina, int filasPorPagina) {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                statementDetalle = DBUtil.getConnection().prepareStatement("select SUM(Cantidad*Costo) AS Costo from ProduccionDetalleTB where IdProduccion = ?");
...
//                }
    ...
    public static Object Reporte_Produccion(String fechaInicio, String fechaFinal, int estado, String idEncargado) {
        ...
        try {
            ...
            statementProduccion = DBUtil.getConnection().prepareStatement("{call Sp_Reporte_Produccion(?,?,?,?)}}");
