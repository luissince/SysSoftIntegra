...
public class SuministroADO {
    ...
    public static String CrudSuministro(SuministroTB suministroTB, ObservableList<PreciosTB> tvPrecios) {
        ...
        try {
            ...
            preparedValidation = dbf.getConnection()
                    .prepareStatement("select IdSuministro from SuministroTB where IdSuministro = ?");
            ...
            if (preparedValidation.executeQuery().next()) {
                ...
                preparedValidation = dbf.getConnection()
                        .prepareStatement("select Clave from SuministroTB where IdSuministro <> ? and Clave = ?");
                preparedValidation.setString(1, suministroTB.getIdSuministro());
                ...
                preparedValidation = dbf.getConnection().prepareStatement(
                        "select NombreMarca from SuministroTB where IdSuministro <> ? and NombreMarca = ?");
                preparedValidation.setString(1, suministroTB.getIdSuministro());
                ...
                preparedSuministro = dbf.getConnection().prepareStatement(
                ...
                preparedPrecios = dbf.getConnection()
                ...
                preparedPrecios = dbf.getConnection().prepareStatement(
                ...
                for (int i = 0; i < tvPrecios.size(); i++) {
                    ...
                    preparedPrecios.setDouble(5,
                            tvPrecios.get(i).getFactor() <= 0 ? 1 : tvPrecios.get(i).getFactor());
                ...
                preparedValidation = dbf.getConnection()
                        .prepareStatement("select NombreMarca from SuministroTB where NombreMarca = ?");
                ...
                codigoSuministro = dbf.getConnection()
                        .prepareCall("{? = call Fc_Suministro_Codigo_Alfanumerico()}");
                ...
                // ? selectFileImage("./img/" + idSuministro + "." +
                // Tools.getFileExtension(suministroTB.getImagenFile()),
                // suministroTB.getImagenFile())
                // : (suministroTB.getImagenTB().equalsIgnoreCase("") ? ""
                // : selectFileImage("./img/" + idSuministro + "." + Tools.getFileExtension(new
                // File(suministroTB.getImagenTB())), new File(suministroTB.getImagenTB())))
                // );
                //
                ...
                preparedAlmacen = dbf.getConnection().prepareStatement("SELECT IdAlmacen FROM AlmacenTB");
                ResultSet resultSet = preparedAlmacen.executeQuery();
                preparedCantidad = dbf.getConnection().prepareStatement(
    ...
    public static Object CrudMasivoSuministro(SuministroTB suministroTB) {
        ...
        try {
            ...
            preparedValidation = dbf.getConnection().prepareStatement("select Clave from SuministroTB where Clave = ?");
            preparedValidation.setString(1, suministroTB.getClave());
            if (preparedValidation.executeQuery().next()) {
                dbf.getConnection().rollback();
    ...
    public static String DeletedInventarioInicial() {
        ...
        try {
            ...
            preparedValidar = dbf.getConnection().prepareStatement("SELECT * FROM VentaTB");
            if (preparedValidar.executeQuery().next()) {
                dbf.getConnection().rollback();
                return "venta";
            } else {
                preparedValidar.close();
                preparedValidar = dbf.getConnection().prepareStatement("SELECT * FROM CompraTB");
                if (preparedValidar.executeQuery().next()) {
                    dbf.getConnection().rollback();
                    return "compra";
                } else {

                    preparedValidar = dbf.getConnection().prepareStatement("SELECT * FROM CotizacionTB");
                    if (preparedValidar.executeQuery().next()) {
                        dbf.getConnection().rollback();
                        return "cotizacion";
                    } else {

                        preparedValidar = dbf.getConnection().prepareStatement("SELECT * FROM ProduccionTB");
                        if (preparedValidar.executeQuery().next()) {
                            dbf.getConnection().rollback();
                            return "produccion";
                        } else {

                            preparedValidar = dbf.getConnection().prepareStatement("SELECT * FROM FormulaTB");
                            if (preparedValidar.executeQuery().next()) {
                                dbf.getConnection().rollback();
                                return "formula";
                            } else {

                                preparedValidar = dbf.getConnection()
                                        .prepareStatement("SELECT * FROM MovimientoInventarioTB");
                                if (preparedValidar.executeQuery().next()) {
                                    dbf.getConnection().rollback();
                                    return "movimiento_inventario";
                                } else {

                                    preparedSuministro = dbf.getConnection()
                                            .prepareStatement("TRUNCATE TABLE SuministroTB");
                                    preparedSuministro.addBatch();
                                    preparedSuministro.executeBatch();

                                    preparedPrecio = dbf.getConnection().prepareStatement("TRUNCATE TABLE PreciosTB");
                                    preparedPrecio.addBatch();
                                    preparedPrecio.executeBatch();

                                    preparedKardex = dbf.getConnection()
                                            .prepareStatement("TRUNCATE TABLE KardexSuministroTB");
                                    preparedKardex.addBatch();
                                    preparedKardex.executeBatch();

                                    preparedCantidad = dbf.getConnection()
                                            .prepareStatement("TRUNCATE TABLE CantidadTB");
                                    preparedCantidad.addBatch();
                                    preparedCantidad.executeBatch();
    ...
    public static Object Listar_Suministros_Lista_View(short tipo, String value, int posicionPagina,
            int filasPorPagina) {
        DBUtil dbf = new DBUtil();
        ...
        try {
            ...
            preparedStatement = dbf.getConnection()
                    .prepareStatement("{call Sp_Listar_Suministros_Lista_View(?,?,?,?)}");
            ...
            while (rsEmps.next()) {
                ...
                ImageView image = new ImageView(new Image(
                        suministroTB.getValorInventario() == 1 ? "/view/image/unidad.png" : "/view/image/balanza.png"));
            ...
            preparedStatement = dbf.getConnection()
                    .prepareStatement("{call Sp_Listar_Suministros_Lista_View_Count(?,?)}");
            preparedStatement.setShort(1, tipo);
            preparedStatement.setString(2, value);
            rsEmps.close();
            rsEmps = preparedStatement.executeQuery();
    ...
    public static Object ListarSuministros(short opcion, String clave, String nombreMarca, int categoria, int marca,
        ...
        try {
            ...
            Object[] objects = new Object[2];
            ObservableList<SuministroTB> empList = FXCollections.observableArrayList();
            preparedSuministros = dbf.getConnection().prepareStatement("{call Sp_Listar_Suministros(?,?,?,?,?,?,?)}");
            ...
            while (rsEmps.next()) {
                ...
                suministroTB
                        .setImpuestoTB(new ImpuestoTB(rsEmps.getString("ImpuestoNombre"), rsEmps.getDouble("Valor")));

                empList.add(suministroTB);
            }

            preparedTotales = dbf.getConnection().prepareStatement("{call Sp_Listar_Suministros_Count(?,?,?,?,?)}");
    ...
    public static SuministroTB GetSuministroById(String value) {
        ...
        try {
            dbf.dbConnect();
            preparedStatement = dbf.getConnection().prepareStatement("{call Sp_Suministro_By_Id(?)}");
    ...
    public static Object[] Validate_Lote_By_IdSuministro(String idSuministro) {
        ...
        try {
            ...
            statementValidate = dbf.getConnection()
                    .prepareStatement("SELECT Cantidad,Lote FROM SuministroTB WHERE IdSuministro = ?");
    ...
    public static String RemoverSuministro(String idSuministro) {
        ...
        try {
            dbf.dbConnect();
            dbf.getConnection().setAutoCommit(false);
            preparedValidate = dbf.getConnection()
                    .prepareStatement("SELECT * FROM DetalleCompraTB WHERE IdArticulo = ?");
            ...
            if (preparedValidate.executeQuery().next()) {
                ...
                preparedValidate = dbf.getConnection()
                        .prepareStatement("SELECT * FROM KardexSuministroTB WHERE IdSuministro = ?");
                preparedValidate.setString(1, idSuministro);
                if (preparedValidate.executeQuery().next()) {
                    dbf.getConnection().rollback();
                    ...
                    preparedValidate = dbf.getConnection()
                            .prepareStatement("SELECT * FROM DetalleVentaTB WHERE IdArticulo = ?");
                    preparedValidate.setString(1, idSuministro);
                    if (preparedValidate.executeQuery().next()) {
                        dbf.getConnection().rollback();
                        result = "venta";
                        ...
                        preparedSuministro = dbf.getConnection()
                                .prepareStatement("DELETE FROM SuministroTB WHERE IdSuministro = ?");
                        preparedSuministro.setString(1, idSuministro);
                        preparedSuministro.addBatch();
                        preparedSuministro.executeBatch();
                        ...
                        preparedSuministro = dbf.getConnection()
                                .prepareStatement("DELETE FROM KardexSuministroTB WHERE IdSuministro = ?");
                        preparedSuministro.setString(1, idSuministro);
                        preparedSuministro.addBatch();
                        preparedSuministro.executeBatch();

                        preparedSuministro = dbf.getConnection()
                                .prepareStatement("DELETE FROM CantidadTB WHERE IdSuministro = ?");
                        preparedSuministro.setString(1, idSuministro);
                        preparedSuministro.addBatch();
                        preparedSuministro.executeBatch();
    ...
    public static ObservableList<PreciosTB> GetItemPriceList(String idSuministro) {
        ...
        try {
            dbf.dbConnect();
            statementVendedor = dbf.getConnection().prepareStatement("{CALL Sp_Listar_Precios_By_IdSuministro(?)}");
    ...
    public static SuministroTB Get_Suministro_By_Search(String value) {
        ...
        try {
            dbf.dbConnect();
            preparedStatement = dbf.getConnection().prepareStatement("{call Sp_Listar_Suministro_By_Search(?)}");
            preparedStatement.setString(1, value);
            rsEmps = preparedStatement.executeQuery();
            if (rsEmps.next()) {
                suministroTB = new SuministroTB();
    ...
    public static List<SuministroTB> getSearchComboBoxSuministros(String buscar, boolean directo) {
        ...
        try {
            dbf.dbConnect();
            preparedStatement = dbf.getConnection().prepareStatement(directo ? "SELECT\n"
                    + "s.IdSuministro,\n"
                    + "s.Clave,\n"
                    + "s.NombreMarca,\n"
                    + "d.Nombre AS Unidad\n"
    ...
    public static String UpdatedInventarioStockMM(int idAlmacen, String idSuministro, String stockMinimo,
        ...
        try {
            ...
            dbf.getConnection().setAutoCommit(false);
            if (idAlmacen == 0) {
                preparedUpdatedSuministro = dbf.getConnection().prepareStatement(
                        "UPDATE SuministroTB SET StockMinimo = ? , StockMaximo = ? WHERE IdSuministro = ?");
                ...
                preparedUpdatedSuministro = dbf.getConnection().prepareStatement(
                        "UPDATE CantidadTB SET StockMinimo = ? , StockMaximo = ? WHERE IdAlmacen = ? AND IdSuministro = ?");
                preparedUpdatedSuministro.setDouble(1, Double.parseDouble(stockMinimo));
                preparedUpdatedSuministro.setDouble(2, Double.parseDouble(stockMaximo));
    ...
    public static Object List_Suministros_Movimiento(String idSuministro) {
        ...
        try {
            dbf.dbConnect();
            preparedStatement = dbf.getConnection().prepareStatement("{call Sp_Get_Suministro_For_Movimiento(?)}");
    ...
    public static Object GetReporteGeneralInventario(int idInventario, int idUnidad, int idCategoria, int idMarca,
        ...
        try {
            ...
            preparedStatementSuministros = dbf.getConnection()
                    .prepareStatement("{call Sp_Reporte_General_Inventario(?,?,?,?,?,?)}");
    ...
    public static Object ListInventario(String producto, int tipoExistencia, String nameProduct, int opcion,
        ...
        try {
            ...
            preparedStatementSuministros = dbf.getConnection()
                    .prepareStatement("{call Sp_Listar_Inventario_Suministros(?,?,?,?,?,?,?,?,?)}");
            ...
            while (rsEmps.next()) {
                ...
                lblCantidad.getStyleClass().add(
                        ...
                        : suministroTB.getCantidad() > 0
                        && suministroTB.getCantidad() < suministroTB.getStockMinimo()
                        ? "label-existencia-intermedia"
                        : suministroTB.getCantidad() >= suministroTB.getStockMinimo()
                        && suministroTB.getCantidad() < suministroTB.getStockMaximo()
            ...
            preparedStatementTotales = dbf.getConnection()
                    .prepareStatement("{call Sp_Listar_Inventario_Suministros_Count(?,?,?,?,?,?,?)}");
