...
public class FormulaADO {
    ...
    public static Object ListarFormulas(int opcion, String fechaInicio, String fechaFinal, String buscar, int posicionPagina, int filasPorPagina) {
        ...
        try {
            ...
            statementLista = dbf.getConnection().prepareStatement("{CALL Sp_Listar_Formulas(?,?,?,?,?,?)}");
            ...
            statementLista = dbf.getConnection().prepareStatement("{CALL Sp_Listar_Formulas_Count(?,?,?,?)}");
            statementLista.setInt(1, opcion);
            statementLista.setString(2, fechaInicio);
            statementLista.setString(3, fechaFinal);
            statementLista.setString(4, buscar);
    ...
    public static String Crud_Formula(FormulaTB formulaTB) {
        ...
        try {
            ...
            statementValidate = dbf.getConnection().prepareStatement("SELECT * FROM FormulaTB WHERE IdFormula = ?");
            statementValidate.setString(1, formulaTB.getIdFormula());
            if (statementValidate.executeQuery().next()) {
                statementFormula = dbf.getConnection().prepareStatement("UPDATE FormulaTB SET Titulo = ?,Cantidad = ?,IdSuministro = ?,CostoAdicional = ?,Instrucciones = ?,IdUsuario = ? WHERE IdFormula = ?");
                ...
                statementDetalle = dbf.getConnection().prepareStatement("DELETE FROM FormulaDetalleTB WHERE IdFormula = ?");
                statementDetalle.setString(1, formulaTB.getIdFormula());
                statementDetalle.addBatch();
                statementDetalle.executeBatch();

                statementDetalle = dbf.getConnection().prepareStatement("INSERT INTO FormulaDetalleTB(IdFormula,IdSuministro,Cantidad) VALUES(?,?,?)");
                ...
                statementCodigo = dbf.getConnection().prepareCall("{? = call Fc_Formula_Alfanumerico()}");
                ...
                statementFormula = dbf.getConnection().prepareStatement("INSERT INTO FormulaTB(IdFormula,Titulo,Cantidad,IdSuministro,CostoAdicional,Instrucciones,Fecha,Hora,IdUsuario) VALUES(?,?,?,?,?,?,?,?,?)");
    ...
    public static Object Obtener_Formula_ById_Editar(String idFormula) {
        ...
        try {
            dbf.dbConnect();
            statementFormula = dbf.getConnection().prepareStatement("{CALL Sp_Obtener_Formula_ById(?)}");
            ...
            if (resultSet.next()) {
                ...
                statementDetalle = dbf.getConnection().prepareStatement("{CALL Sp_Obtener_Detalle_Formula_ById(?)}");
...
//                    button.setMaxHeight(Double.MAX_VALUE);
    ...
    public static Object Obtener_Formula_ByIdProducto(String idProducto) {
        ...
        try {
            dbf.dbConnect();
            statementFormula = dbf.getConnection().prepareStatement("SELECT IdFormula,Titulo FROM FormulaTB WHERE IdSuministro = ?");
    ...
    public static String Eliminar_Formula_ById(String idFormula) {
        ...
        try {
            ...
            statementValidar = dbf.getConnection().prepareStatement("SELECT * FROM FormulaTB WHERE IdFormula = ?");
            statementValidar.setString(1, idFormula);
            if (statementValidar.executeQuery().next()) {
                statementFormula = dbf.getConnection().prepareStatement("DELETE FROM FormulaTB WHERE IdFormula = ?");
                statementFormula.setString(1, idFormula);
                statementFormula.addBatch();
                statementFormula.executeBatch();
    ...
    public static List<FormulaTB> getSearchComboBoxFormulas() {
        ...
        try {
            dbf.dbConnect();
            preparedStatement = dbf.getConnection().prepareStatement( "SELECT f.IdFormula,f.Titulo,s.IdSuministro,s.Clave,s.NombreMarca FROM FormulaTB AS f INNER JOIN SuministroTB AS s ON f.IdSuministro = s.IdSuministro");
